...
The `mts web` command provides a (preview) web-based chat interface for interacting with mts.
...
mts web --host 0.0.0.0 --port 8080
```

## Features

- **Real-time chat interface**: Communicate with mts through a clean web UI
...
### Integration with mts Agent

The web server creates an instance of the mts Agent and processes messages through the same pipeline as the CLI. However, some features like:
...
3. **Agent not configured**: Run `mts configure` first to set up a provider