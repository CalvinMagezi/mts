...
use mts::providers::{create, testprovider::TestProvider};
...
{
    if let Ok(only_provider) = std::env::var("MTS_TEST_PROVIDER") {
...
{
    use mts::config::ExtensionConfig;
    use tokio::sync::Mutex;

    mts::agents::moim::SKIP.with(|f| f.set(true));
    ...
    agent
        .update_provider(
            provider_arc as Arc<dyn mts::providers::base::Provider>,
            &session.id,
        )
        .await?;
