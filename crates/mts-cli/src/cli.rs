...
use mts_bench::bench_config::BenchRunConfig;
use mts_bench::runners::bench_runner::BenchRunner;
use mts_bench::runners::eval_runner::EvalRunner;
use mts_bench::runners::metric_aggregator::MetricAggregator;
use mts_bench::runners::model_runner::ModelRunner;
use std::io::Read;
...
enum Command {
    ...
    Run {
        ...
        #[arg(
            ...
            long_help = "Override the MTS_PROVIDER environment variable for this run. Available providers include openai, anthropic, ollama, databricks, gemini-cli, claude-code, and others."
        )]
        ...
        #[arg(
            ...
            long_help = "Override the MTS_MODEL environment variable for this run. The model must be supported by the specified provider."
        )]
        model: Option<String>,
    },
...
pub struct RecipeInfo {
    ...
    pub sub_recipes: Option<Vec<mts::recipe::SubRecipe>>,
    pub final_output_response: Option<mts::recipe::Response>,
    pub retry_config: Option<mts::agents::types::RetryConfig>,
}
                ...
                (_, _, Some(recipe_name)) => {
                    ...
                    let recipe_version =
                        crate::recipes::search_recipe::load_recipe_file(&recipe_name)
                            .ok()
                            .and_then(|rf| {
                                mts::recipe::template_recipe::parse_recipe_content(
                                    &rf.content,
