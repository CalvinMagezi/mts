...
use mts::tracing::{langfuse_layer, otlp_layer};
use mts_bench::bench_session::BenchAgentError;
use mts_bench::error_capture::ErrorCaptureLayer;
...
fn setup_logging_internal(
    ...
    let mut setup = || {
        result = (|| {
            let log_dir = mts::logging::prepare_log_directory("cli", true)?;
...
mod tests {
    ...
    fn test_log_directory_creation() {
        ...
        let log_dir = mts::logging::prepare_log_directory("cli", true).unwrap();
