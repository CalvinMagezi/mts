...
pub fn extract_recipe_info_from_cli(
    ...
    let recipe_info = RecipeInfo {
        session_settings: recipe.settings.map(|s| SessionSettings {
            mts_provider: s.mts_provider,
            mts_model: s.mts_model,
...
mod tests {
    ...
    fn test_extract_recipe_info_from_cli_basic() {
        ...
        assert_eq!(settings.mts_provider, Some("test_provider".to_string()));
        assert_eq!(settings.mts_model, Some("test_model".to_string()));
...
settings:
  mts_provider: test_provider
  mts_model: test_model
  temperature: 0.7
sub_recipes:
