...
thread_local! {
    static CURRENT_THEME: RefCell<Theme> = RefCell::new(
        std::env::var("MTS_CLI_THEME").ok()
            .map(|val| Theme::from_config_str(&val))
            .unwrap_or_else(||
                Config::global().get_param::<String>("MTS_CLI_THEME").ok()
                    .map(|val| Theme::from_config_str(&val))
...
pub fn set_theme(theme: Theme) {
    ...
    config
        .set_param("MTS_CLI_THEME", theme.as_config_string())
    ...
    if let Err(e) = config.set_param("MTS_CLI_THEME", theme_str) {
    ...
    for content in &message.content {
        match content {
            ...
            MessageContent::SystemNotification(notification) => {
                use mts::conversation::message::SystemNotificationType;
...
pub fn mts_mode_message(text: &str) {
    println!("\n{}", style(text).yellow(),);
}
...
pub fn display_session_info(
    ...
    provider_instance: Option<&Arc<dyn mts::providers::base::Provider>>,
) {
