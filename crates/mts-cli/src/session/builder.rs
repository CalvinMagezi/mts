...
use console::style;
use mts::agents::types::{RetryConfig, SessionConfig};
...
use mts::recipe::{Response, SubRecipe};

use mts::agents::extension::PlatformExtensionContext;
use mts::session::session_manager::SessionType;
use mts::session::SessionManager;
use mts::session::{EnabledExtensionsState, ExtensionState};
...
/// Configuration for building a new MTS session
///
...
pub struct SessionBuilderConfig {
    ...
    /// Settings to override the global MTS settings
...
async fn offer_extension_debugging_help(
    extension_name: &str,
    error_message: &str,
    provider: Arc<dyn mts::providers::base::Provider>,
    interactive: bool,
) -> Result<(), anyhow::Error> {
...
pub struct SessionSettings {
    pub mts_model: Option<String>,
    pub mts_provider: Option<String>,
    pub temperature: Option<f32>,
}

pub async fn build_session(session_config: SessionBuilderConfig) -> CliSession {
    mts::posthog::set_session_context("cli", session_config.resume);
    ...
    let provider_name = session_config
        ...
        .or_else(|| {
            session_config
                .settings
                .as_ref()
                .and_then(|s| s.mts_provider.clone())
        })
        .or_else(|| config.get_mts_provider().ok())
    ...
    let model_name = session_config
        ...
        .or_else(|| {
            session_config
                .settings
                .as_ref()
                .and_then(|s| s.mts_model.clone())
        })
        .or_else(|| config.get_mts_model().ok())
            ...
            .is_some_and(|mc| mc.model_name == model_name)
    {
        ...
        mts::model::ModelConfig::new(&model_name)
            .unwrap_or_else(|e| {
    ...
    let new_provider = match create(&provider_name, model_config).await {
        ...
        Err(e) => {
            output::render_error(&format!(
                ...
                For more info, see: https://block.github.io/mts/docs/troubleshooting/#keychainkeyring-errors",
                e
            ));
            process::exit(1);
        }
    ...
    let debug_mode = session_config.debug || config.get_param("MTS_DEBUG").unwrap_or(false);
    ...
    let system_prompt_file: Option<String> = config.get_param("MTS_SYSTEM_PROMPT_FILE_PATH").ok();
