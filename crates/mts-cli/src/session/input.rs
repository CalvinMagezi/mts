use super::completion::MTSCompleter;
use anyhow::Result;
...
pub fn get_input(
    editor: &mut Editor<MTSCompleter, rustyline::history::DefaultHistory>,
) -> Result<InputResult> {
...
fn handle_slash_command(input: &str) -> Option<InputResult> {
    ...
    match input {
        ...
        s if s.starts_with(CMD_MODE) => Some(InputResult::MtsMode(
...
/// Returns a styled prompt with the mts face "( O)>" followed by a space.
...
fn get_input_prompt_string() -> String {
    ...
    if cfg!(target_os = "windows") {
        ...
        format!("{} ", console::style(mts).cyan().bold())
    }
}
...
/mode <name> - Set the mts mode to use ('auto', 'approve', 'chat', 'smart_approve')
/plan <message_text> -  Enters 'plan' mode with optional message. Create a plan based on the current messages and asks user if they want to act on it.
                        If user acts on the plan, mts mode is set to 'auto' and returns to 'normal' mts mode.
                        To warm up mts before using '/plan', we recommend setting '/mode approve' & putting appropriate context into mts.
                        The model is used based on $MTS_PLANNER_PROVIDER and $MTS_PLANNER_MODEL environment variables.
                        If no model is set, the default model is used.
/endplan - Exit plan mode and return to 'normal' mts mode.
...
mod tests {
    ...
    fn test_get_input_prompt_string() {
        ...
        // Prompt should contain the mts face
