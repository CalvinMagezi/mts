...
use console::Color;
use mts::agents::AgentEvent;
use mts::permission::permission_confirmation::PrincipalType;
use mts::permission::Permission;
use mts::permission::PermissionConfirmation;
use mts::providers::base::Provider;
...
use mts::conversation::message::{ActionRequiredData, Message, MessageContent};
...
impl CliSession {
    ...
    pub async fn add_extension(&mut self, extension_command: String) -> Result<()> {
        ...
        let config = ExtensionConfig::Stdio {
            ...
            envs: Envs::new(envs),
            env_keys: Vec::new(),
            description: mts::config::DEFAULT_EXTENSION_DESCRIPTION.to_string(),
    ...
    pub async fn interactive(&mut self, prompt: Option<String>) -> Result<()> {
        ...
        loop {
            ...
            match input::get_input(&mut editor)? {
                ...
                input::InputResult::MtsMode(mode) => {
                    save_history(&mut editor);

                    let config = Config::global();
                    let mode = match MtsMode::from_str(&mode.to_lowercase()) {
                        Ok(mode) => mode,
                        Err(_) => {
            ...
            PlannerResponseType::Plan => {
                ...
                if should_act {
                    ...
                    if curr_mts_mode != MtsMode::Auto {
                        config.set_mts_mode(MtsMode::Auto).unwrap();
                    }
                ...
                result = stream.next() => {
                    match result {
                                                ...
                                                let formatted = match notification_type {
                                                    ...
                                                    Some("response_generated") => {
                                                        ...
                                                        let min_priority = config
                                                            .get_param::<f32>("MTS_CLI_MIN_PRIORITY")
                                                            .ok()
                                                            .unwrap_or(0.5);
                        ...
                        Some(Err(e)) => {
                            ...
                            if e.downcast_ref::<mts::providers::errors::ProviderError>()
                                .map(|provider_error| matches!(provider_error, mts::providers::errors::ProviderError::ContextLengthExceeded(_)))
                                .unwrap_or(false) {
    ...
    pub async fn get_session(&self) -> Result<mts::session::Session> {
    ...
    pub async fn display_context_usage(&self) -> Result<()> {
        ...
        let show_cost = config
            .get_param::<bool>("MTS_CLI_SHOW_COST")
            .unwrap_or(false);
...
async fn get_reasoner() -> Result<Arc<dyn Provider>, anyhow::Error> {
    ...
    let provider = if let Ok(provider) = config.get_param::<String>("MTS_PLANNER_PROVIDER") {
        provider
    } else {
    ...
    let model = if let Ok(model) = config.get_param::<String>("MTS_PLANNER_MODEL") {
        model
    } else {
