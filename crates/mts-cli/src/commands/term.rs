...
use chrono;
use mts::conversation::message::{Message, MessageContent, MessageMetadata};
use mts::session::SessionManager;
use mts::session::SessionType;
use rmcp::model::Role;
...
pub async fn handle_term_info() -> Result<()> {
    ...
    let context_limit = config
        .get_mts_model()
        .ok()
        .and_then(|model_name| mts::model::ModelConfig::new(&model_name).ok())
