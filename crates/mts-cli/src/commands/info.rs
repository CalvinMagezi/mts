...
use mts::session::session_manager::{DB_NAME, SESSIONS_FOLDER};
use serde_yaml;
...
use mts::config::base::CONFIG_YAML_NAME;
use std::fs;
use std::path::Path;
...
pub fn handle_info(verbose: bool) -> Result<()> {
    ...
    println!("{}", style("mts Version:").cyan().bold());
    ...
    if verbose {
        println!("\n{}", style("mts Configuration:").cyan().bold());
