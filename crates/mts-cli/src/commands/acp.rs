...
use anyhow::Result;
use mts::agents::{Agent, SessionConfig};
use mts::config::{get_all_extensions, Config};
use mts::conversation::message::{Message, MessageContent};
use mts::conversation::Conversation;
use mts::mcp_utils::ToolResult;
...
impl MTSAcpAgent {
    async fn new(
        ...
        let model_config = mts::model::ModelConfig {
...
impl acp::Agent for MTSAcpAgent {
    ...
    async fn prompt(&self, args: acp::PromptRequest) -> Result<acp::PromptResponse, acp::Error> {
        ...
        while let Some(event) = stream.next().await {
            ...
            match event {
                Ok(mts::agents::AgentEvent::Message(message)) => {
                    let mut sessions = self.sessions.lock().await;
                    let session = sessions
                        .get_mut(&session_id)
                        .ok_or_else(acp::Error::invalid_params)?;
