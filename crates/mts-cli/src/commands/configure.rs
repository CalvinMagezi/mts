use crate::recipes::github_recipe::MTS_RECIPE_GITHUB_REPO_CONFIG_KEY;
use cliclack::spinner;
use console::style;
use mts::agents::extension::ToolInfo;
use mts::agents::extension_manager::get_parameter_names;
use mts::agents::Agent;
use mts::agents::{extension::Envs, ExtensionConfig};
use mts::config::declarative_providers::{create_custom_provider, remove_custom_provider};
use mts::config::extensions::{
    get_all_extension_names, get_all_extensions, get_enabled_extensions, get_extension_by_name,
    name_to_key, remove_extension, set_extension, set_extension_enabled,
};
use mts::config::paths::Paths;
use mts::config::permission::PermissionLevel;
use mts::config::signup_tetrate::TetrateAuth;
use mts::config::{
    configure_tetrate, Config, ConfigError, ExperimentManager, ExtensionEntry, MtsMode,
    PermissionManager,
};
use mts::conversation::message::Message;
use mts::model::ModelConfig;
use mts::providers::provider_test::test_provider_configuration;
use mts::providers::{create, providers};
use mts::session::{SessionManager, SessionType};
...
fn select_model_from_list(
    models: &[String],
    provider_meta: &mts::providers::base::ProviderMetadata,
    ...
    match config.set_secret(key_name, &value) {
        ...
        Err(e) => {
            cliclack::outro(style(format!(
                "Failed to store {} securely: {}. Please ensure your system's secure storage is accessible. Alternatively you can run with MTS_DISABLE_KEYRING=true or set the key in your environment variables",
                key_name, e
            )).on_red().white())?;
            Ok(false)
        }
...
pub async fn configure_provider_dialog() -> anyhow::Result<bool> {
    ...
    let current_provider: Option<String> = config.get_mts_provider().ok();
    ...
    let model: String = match models_res {
        ...
        Ok(None) => {
            let default_model =
                std::env::var("MTS_MODEL").unwrap_or(provider_meta.default_model.clone());
    ...
    let toolshim_enabled = std::env::var("MTS_TOOLSHIM")
        .map(|val| val == "1" || val.to_lowercase() == "true")
        .unwrap_or(false);
    let toolshim_model = std::env::var("MTS_TOOLSHIM_OLLAMA_MODEL").ok();

    match test_provider_configuration(provider_name, &model, toolshim_enabled, toolshim_model).await
    {
        Ok(()) => {
            config.set_mts_provider(provider_name)?;
            config.set_mts_model(&model)?;
    ...
    match extension_type {
        ...
        "built-in" => {
            ...
            let timeout: u64 = cliclack::input("Please set the timeout for this tool (in secs):")
                .placeholder(&mts::config::DEFAULT_EXTENSION_TIMEOUT.to_string())
...
pub fn configure_mts_mode_dialog() -> anyhow::Result<()> {
    let config = Config::global();

    if std::env::var("MTS_MODE").is_ok() {
    ...
    let msg = match mode {
        MtsMode::Auto => "Set to Auto Mode - full file modification enabled",
        MtsMode::Approve => "Set to Approve Mode - all tools and modifications require approval",
        MtsMode::SmartApprove => "Set to Smart Approve Mode - modifications require approval",
        MtsMode::Chat => "Set to Chat Mode - no tools or modifications enabled",
    };
    cliclack::outro(msg)?;
    Ok(())
}
...
pub fn configure_mts_router_strategy_dialog() -> anyhow::Result<()> {
    ...
    config.set_param("MTS_ENABLE_ROUTER", enable_router)?;
...
pub fn configure_tool_output_dialog() -> anyhow::Result<()> {
    ...
    if std::env::var("MTS_CLI_MIN_PRIORITY").is_ok() {
        let _ = cliclack::log::info("Notice: MTS_CLI_MIN_PRIORITY environment variable is set and will override the configuration here.");
    }
    ...
    match tool_log_level {
        "high" => {
            config.set_param("MTS_CLI_MIN_PRIORITY", 0.8)?;
            cliclack::outro("Showing tool output of high importance only.")?;
        }
        "medium" => {
            config.set_param("MTS_CLI_MIN_PRIORITY", 0.2)?;
            cliclack::outro("Showing tool output of medium importance.")?;
        }
        "all" => {
            config.set_param("MTS_CLI_MIN_PRIORITY", 0.0)?;
...
pub fn configure_keyring_dialog() -> anyhow::Result<()> {
    ...
    if std::env::var("MTS_DISABLE_KEYRING").is_ok() {
        let _ = cliclack::log::info("Notice: MTS_DISABLE_KEYRING environment variable is set and will override the configuration here.");
    }

    let currently_disabled = config.get_param::<String>("MTS_DISABLE_KEYRING").is_ok();
...
fn configure_recipe_dialog() -> anyhow::Result<()> {
    let key_name = MTS_RECIPE_GITHUB_REPO_CONFIG_KEY;
    let config = Config::global();
...
pub fn configure_max_turns_dialog() -> anyhow::Result<()> {
    ...
    let current_max_turns: u32 = config.get_param("MTS_MAX_TURNS").unwrap_or(1000);
...
pub async fn handle_openrouter_auth() -> anyhow::Result<()> {
    use mts::config::{configure_openrouter, signup_openrouter::OpenRouterAuth};
    use mts::conversation::message::Message;
    use mts::providers::create;
    ...
    let configured_model: String = config.get_mts_model()?;
    let model_config = match mts::model::ModelConfig::new(&configured_model) {
        Ok(config) => config,
        Err(e) => {
                    ...
                    if !has_developer {
                        set_extension(ExtensionEntry {
                            ...
                            config: ExtensionConfig::Builtin {
                                ...
                                display_name: Some(mts::config::DEFAULT_DISPLAY_NAME.to_string()),
                                timeout: Some(mts::config::DEFAULT_EXTENSION_TIMEOUT),
                                bundled: Some(true),
...
fn remove_provider() -> anyhow::Result<()> {
    let custom_providers_dir = mts::config::declarative_providers::custom_providers_dir();
    let custom_providers = if custom_providers_dir.exists() {
        mts::config::declarative_providers::load_custom_providers(&custom_providers_dir)?
    } else {
        Vec::new()
    };
