...
pub async fn handle_web(
    ...
    let model_config = mts::model::ModelConfig::new(&model)?;
    ...
    let agent = Agent::new();
    let provider = mts::providers::create(&provider_name, model_config).await?;
...
async fn serve_session(
    ...
    let html_with_session = html.replace(
        ...
        &format!(
            "<script>window.MTS_SESSION_NAME = '{}'; window.MTS_WS_TOKEN = '{}';</script>\n    <script src=\"/static/script.js\"></script>",
            session_name,
            state.ws_token
        )
    );
