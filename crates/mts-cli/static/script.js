...
function getSessionId() {
    ...
    if (window.MTS_SESSION_NAME) {
        return window.MTS_SESSION_NAME;
    }
...
function addThinkingIndicator() {
    ...
    thinkingDiv.innerHTML = `
        ...
        <span class="thinking-text">mts is thinking...</span>
    `;
...
function connectWebSocket() {
    ...
    const token = window.MTS_WS_TOKEN || '';
        ...
        if (response.ok) {
            ...
            if (sessionData.messages && sessionData.messages.length > 0) {
                ...
                if (sessionData.metadata && sessionData.metadata.description) {
                    document.title = `mts chat - ${sessionData.metadata.description}`;
                }
...
function updateSessionTitle() {
    ...
    // Just show "mts chat" - no need to show session ID
    titleElement.textContent = 'mts chat';
}
