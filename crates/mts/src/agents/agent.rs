...
use crate::config::{get_enabled_extensions, Config, MtsMode};
...
impl Agent {
    ...
    async fn prepare_reply_context(
        ...
        self.tool_inspection_manager
            .update_permission_inspector_mode(mts_mode)
            .await;
    ...
    pub async fn add_sub_recipes(&self, sub_recipes: Vec<SubRecipe>) {
        let mut sub_recipes_map = self.sub_recipes.lock().await;
        for sub_recipe in sub_recipes {
            sub_recipes_map.insert(sub_recipe.name.clone(), sub_recipe);
        }
    }
    ...
    pub async fn list_tools(&self, extension_name: Option<String>) -> Vec<Tool> {
        ...
        if extension_name.is_none() {
            ...
            let sub_recipes_vec: Vec<SubRecipe> = sub_recipes.values().cloned().collect();

            // Add unified subagent tool with subrecipes
            ...
            let final_conversation = if !needs_auto_compact && !is_manual_compact {
                ...
                if !is_manual_compact {
                    ...
                    let threshold = config
                        .get_param::<f64>("MTS_AUTO_COMPACT_THRESHOLD")
    ...
    pub async fn create_recipe(&self, mut messages: Conversation) -> Result<Recipe> {
        ...
        let settings = Settings {
            ...
            mts_model: Some(model_name.clone()),
