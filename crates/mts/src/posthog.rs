...
pub const TELEMETRY_ENABLED_KEY: &str = "MTS_TELEMETRY_ENABLED";

static TELEMETRY_DISABLED_BY_ENV: Lazy<AtomicBool> = Lazy::new(|| {
    std::env::var("MTS_TELEMETRY_OFF")
...
///
/// Returns false if:
/// - MTS_TELEMETRY_OFF environment variable is set to "1" or "true"
/// - MTS_TELEMETRY_ENABLED config value is set to false
///
...
fn detect_install_method() -> String {
    ...
    if std::env::var("MTS_DESKTOP").is_ok() {
        return "desktop".to_string();
    }
...
async fn send_error_event(installation: &InstallationData, error_type: &str) -> Result<(), String> {
    ...
    let config = Config::global();
    if let Ok(provider) = config.get_param::<String>("MTS_PROVIDER") {
        event.insert_prop("provider", provider).ok();
    }
    if let Ok(model) = config.get_param::<String>("MTS_MODEL") {
