...
mod tests {
    ...
    fn test_mtshints_when_present() {
        ...
        let hints = load_hint_files(dir.path(), &[MTS_HINTS_FILENAME.to_string()], &gitignore);
    ...
    fn test_mtshints_multiple_filenames() {
        ...
        let hints = load_hint_files(
            dir.path(),
            &["CLAUDE.md".to_string(), MTS_HINTS_FILENAME.to_string()],
            &gitignore,
        );
    ...
    fn test_nested_mtshints_with_git_root() {
        ...
        fs::write(project_root.join(MTS_HINTS_FILENAME), "Root hints content").unwrap();

        let subdir = project_root.join("subdir");
        fs::create_dir(&subdir).unwrap();
        fs::write(subdir.join(MTS_HINTS_FILENAME), "Subdir hints content").unwrap();
        let current_dir = subdir.join("current_dir");
        fs::create_dir(&current_dir).unwrap();
        fs::write(
            current_dir.join(MTS_HINTS_FILENAME),
            "current_dir hints content",
        )
        .unwrap();

        let gitignore = create_dummy_gitignore();
        let hints = load_hint_files(&current_dir, &[MTS_HINTS_FILENAME.to_string()], &gitignore);
    ...
    fn test_nested_mtshints_without_git_root() {
        ...
        fs::write(base_dir.join(MTS_HINTS_FILENAME), "Base hints content").unwrap();
    ...
    fn test_nested_mtshints_mixed_filenames() {
        ...
        let subdir = project_root.join("subdir");
        fs::create_dir(&subdir).unwrap();
        fs::write(subdir.join(MTS_HINTS_FILENAME), "Subdir .mtshints content").unwrap();
...
Additional instructions here."#;
        fs::write(project_root.join(MTS_HINTS_FILENAME), hints_content).unwrap();

        let gitignore = create_dummy_gitignore();
        let hints = load_hint_files(project_root, &[MTS_HINTS_FILENAME.to_string()], &gitignore);
...
Root level instructions"#;
        fs::write(project_root.join(MTS_HINTS_FILENAME), root_hints_content).unwrap();
...
End of nested hints"#;
        fs::write(
            components_dir.join(MTS_HINTS_FILENAME),
...
@../parent.md
End of hints"#;
        fs::write(current_dir.join(MTS_HINTS_FILENAME), hints_content).unwrap();
...
@../root_file.md
End of hints"#;
        fs::write(subdir.join(MTS_HINTS_FILENAME), hints_content).unwrap();
        let gitignore = create_dummy_gitignore();

        let hints = load_hint_files(&subdir, &[MTS_HINTS_FILENAME.to_string()], &gitignore);
