use mts::agents::recipe_tools::dynamic_task_tools::{
...
mod tests {
    ...
    async fn test_create_multiple_tasks() {
        use mts::agents::subagent_execution_tool::tasks_manager::TasksManager;
    ...
    fn test_with_settings() {
        ...
        assert_eq!(settings.mts_provider, Some("openai".to_string()));
        assert_eq!(settings.mts_model, Some("gpt-4".to_string()));
    ...
    fn test_extension_shortnames() {
        ...
        if !extensions.is_empty() {
            ...
            assert!(matches!(
                &extensions[0],
                mts::agents::extension::ExtensionConfig::Builtin { .. }
                    | mts::agents::extension::ExtensionConfig::Stdio { .. }
                    | mts::agents::extension::ExtensionConfig::Sse { .. }
                    | mts::agents::extension::ExtensionConfig::StreamableHttp { .. }
                    | mts::agents::extension::ExtensionConfig::Frontend { .. }
                    | mts::agents::extension::ExtensionConfig::InlinePython { .. }
            ));
        }
    }
    ...
    fn test_mixed_extension_formats() {
        ...
        if let Some(last) = extensions.last() {
            match last {
                mts::agents::extension::ExtensionConfig::Stdio { name, .. } => {
    ...
    fn test_unknown_extension_shortname() {
        ...
        match &extensions[0] {
            mts::agents::extension::ExtensionConfig::Builtin { name, .. } => {
