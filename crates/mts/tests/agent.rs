...
use mts::config::extensions::{set_extension, ExtensionEntry};
...
mod tests {
    ...
    mod schedule_tool_tests {
        ...
        use mts::agents::platform_tools::PLATFORM_MANAGE_SCHEDULE_TOOL_NAME;
        use mts::scheduler::{ScheduledJob, SchedulerError};
        use mts::scheduler_trait::SchedulerTrait;
        use mts::session::Session;
    ...
    mod retry_tests {
        use super::*;
        use mts::agents::types::{RetryConfig, SuccessCheck};

        #[tokio::test]
        async fn test_retry_success_check_execution() -> Result<()> {
            use mts::agents::retry::execute_success_checks;
    ...
    mod max_turns_tests {
        ...
        use mts::conversation::message::{Message, MessageContent};
        use mts::model::ModelConfig;
        use mts::providers::base::{Provider, ProviderMetadata, ProviderUsage, Usage};
                ...
                match response_result {
                    Ok(AgentEvent::Message(response)) => {
                            ...
                            response.content.first()
                        {
                            ...
                            req.id.clone(),
                            mts::permission::PermissionConfirmation {
                                principal_type: mts::permission::permission_confirmation::PrincipalType::Tool,
                                permission: mts::permission::Permission::AllowOnce,
                            }
                        ).await;
                        }
    ...
    mod extension_manager_tests {
        use super::*;
        use mts::agents::extension::{ExtensionConfig, PlatformExtensionContext};
        use mts::agents::extension_manager_extension::{
