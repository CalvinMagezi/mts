...
pub(crate) fn to_env_var(field_path: &str) -> String {
    ...
    format!("MTS_{}", normalized_path.replace('.', "__").to_uppercase())
}
...
mod tests {
    ...
    fn test_env_var_conversion() {
        assert_eq!(to_env_var("type"), "MTS_PROVIDER__TYPE");
        assert_eq!(to_env_var("api_key"), "MTS_PROVIDER__API_KEY");
        assert_eq!(to_env_var("provider.host"), "MTS_PROVIDER__HOST");
        assert_eq!(to_env_var("provider.api_key"), "MTS_PROVIDER__API_KEY");
    }
}
