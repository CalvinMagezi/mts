...
use mts::config::{Config, ConfigError};
use mts::model::ModelConfig;
use mts::providers::auto_detect::detect_provider_from_api_key;
use mts::providers::base::{ProviderMetadata, ProviderType};
use mts::providers::create_with_default_model;
use mts::providers::pricing::{
...
pub async fn get_extensions() -> Result<Json<ExtensionResponse>, StatusCode> {
    let extensions = mts::config::get_all_extensions();
...
pub async fn add_extension(
    ...
    let extensions = mts::config::get_all_extensions();
    let key = mts::config::extensions::name_to_key(&extension_query.name);

    let is_update = extensions.iter().any(|e| e.config.key() == key);

    mts::config::set_extension(ExtensionEntry {
...
pub async fn remove_extension(Path(name): Path<String>) -> Result<Json<String>, StatusCode> {
    let key = mts::config::extensions::name_to_key(&name);
    mts::config::remove_extension(&key);
...
pub async fn get_provider_models(
    ...
    let loaded_provider = mts::config::declarative_providers::load_provider(name.as_str()).ok();
    ...
    let provider = mts::providers::create(&name, model_config)
        .await
    ...
    match models_result {
        ...
        Err(provider_error) => {
            use mts::providers::errors::ProviderError;
...
pub async fn init_config() -> Result<Json<String>, StatusCode> {
    ...
    match mts::config::base::load_init_config_from_workspace() {
...
pub async fn upsert_permissions(
    ...
    let mut permission_manager = mts::config::PermissionManager::default();
...
pub async fn create_custom_provider(
    ...
    let config = mts::config::declarative_providers::create_custom_provider(
        &request.engine,
    ...
    if let Err(e) = mts::providers::refresh_custom_providers().await {
...
pub async fn get_custom_provider(
    ...
    let loaded_provider = mts::config::declarative_providers::load_provider(id.as_str())
...
pub async fn remove_custom_provider(Path(id): Path<String>) -> Result<Json<String>, StatusCode> {
    mts::config::declarative_providers::remove_custom_provider(&id)
        .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
...
pub async fn update_custom_provider(
    ...
    mts::config::declarative_providers::update_custom_provider(
        &id,
        &request.engine,
        request.display_name,
        request.api_url,
...
pub async fn set_config_provider(
    ...
    create_with_default_model(&provider)
        ...
        .and_then(|_| {
            let config = Config::global();
            config
                .set_mts_provider(provider)
                .and_then(|_| config.set_mts_model(model))
...
mod tests {
    ...
    async fn test_read_model_limits() {
        ...
        let limits: Vec<mts::model::ModelLimitConfig> = serde_json::from_value(response).unwrap();
