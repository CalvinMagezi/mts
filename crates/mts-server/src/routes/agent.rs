...
use mts::config::PermissionManager;

use mts::agents::ExtensionConfig;
use mts::config::{Config, MtsMode};
use mts::model::ModelConfig;
use mts::prompt_template::render_global_file;
use mts::providers::create;
use mts::recipe::Recipe;
use mts::recipe_deeplink;
use mts::session::session_manager::SessionType;
use mts::session::{Session, SessionManager};
use mts::{
...
async fn start_agent(
    ...
    mts::posthog::set_session_context("desktop", false);
...
async fn resume_agent(
    ...
    mts::posthog::set_session_context("desktop", true);
    ...
    if payload.load_model_and_extensions {
        ...
        let provider_result = async {
            ...
            let model_config = match session.model_config.clone() {
                ...
                None => {
                    let model_name = config.get_mts_model().map_err(|_| ErrorResponse {
        ...
        let extensions_result = async {
            let enabled_configs = mts::config::get_enabled_extensions();
...
async fn get_tools(
    ...
    let config = Config::global();
    let mts_mode = config.get_mts_mode().unwrap_or(MtsMode::Auto);
    ...
    let mut tools: Vec<ToolInfo> = agent
        ...
        .map(|tool| {
            let permission = permission_manager
                ...
                .or_else(|| {
                    if mts_mode == MtsMode::SmartApprove {
                        permission_manager.get_smart_approve_permission(&tool.name)
                    } else if mts_mode == MtsMode::Approve {
...
async fn update_agent_provider(
    ...
    let config = Config::global();
    let model = match payload.model.or_else(|| config.get_mts_model().ok()) {
        Some(m) => m,
        None => {
