#!/usr/bin/env bash
# run-benchmarks.sh - Script to run mts benchmarks across multiple provider:model pairs
...
function show_usage() {
  ...
  echo "  -t, --toolshim           Enable toolshim mode by setting MTS_TOOLSHIM=1"
  echo "  -m, --toolshim-model     Set the toolshim model (sets MTS_TOOLSHIM_MODEL)"
...
MTS_CMD="mts"
if [ "$DEBUG_MODE" = true ]; then
  if [ -f "./target/debug/mts" ]; then
    MTS_CMD="./target/debug/mts"
    echo "Using debug binary: $MTS_CMD"
  else
    echo "Warning: Debug binary not found at ./target/debug/mts. Falling back to system-installed mts."
  fi
else
  if [ -f "./target/release/mts" ]; then
    MTS_CMD="./target/release/mts"
    echo "Using release binary: $MTS_CMD"
  else
    echo "Warning: Release binary not found at ./target/release/mts. Falling back to system-installed mts."
  fi
fi
...
for ((i=0; i<$COUNT; i++)); do
  ...
  if [ "$TOOLSHIM" = true ]; then
    export MTS_TOOLSHIM=1
    if [[ -n "$TOOLSHIM_MODEL" ]]; then
      export MTS_TOOLSHIM_OLLAMA_MODEL="$TOOLSHIM_MODEL"
    fi
  fi
  ...
  if $MTS_CMD bench --suites "$SUITES" --output "$OUTPUT_FILE" --format json; then
