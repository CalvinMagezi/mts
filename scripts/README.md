# MTS Benchmark Scripts

This directory contains scripts for running and analyzing MTS benchmarks.

## run-benchmarks.sh

This script runs MTS benchmarks across multiple provider:model pairs and analyzes the results.

### Prerequisites

- MTS CLI must be built or installed
...
3. For each provider:model pair:
   - Sets the `MTS_PROVIDER` and `MTS_MODEL` environment variables
