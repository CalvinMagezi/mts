{
  description = "mts - An AI agent CLI";
  ...
  outputs = { self, nixpkgs, flake-utils, rust-overlay }:
    flake-utils.lib.eachDefaultSystem (system:
      let
        ...
        cargoToml = builtins.fromTOML (builtins.readFile ./crates/mts-cli/Cargo.toml);
      ...
      in
      {
        defaultPackage = pkgs.rustPlatform.buildRustPackage {
          ...
          cargoBuildFlags = [ "--package" "mts-cli" ];
          ...
          checkPhase = ''
            ...
            # Run tests for mts-cli package only
            cargo test --package mts-cli --release
          '';
        ...
        devShell = pkgs.mkShell {
          ...
          shellHook = ''
            echo "mts development environment"
            echo "Rust version: $(rustc --version)"
            echo ""
            echo "Commands:"
            echo "  nix build           - Build mts CLI"
            echo "  nix run             - Run mts CLI"
            echo "  cargo build -p mts-cli - Build with cargo"
            echo "  cargo run -p mts-cli   - Run with cargo"
          '';
        };
      }
    );
