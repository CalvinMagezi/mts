...
describe('OllamaSetup', () => {
  ...
  describe.skip('when Ollama and model are both available', () => {
    ...
    it('should handle successful connection', async () => {
      ...
      await waitFor(() => {
        fireEvent.click(screen.getByText(/Use MTS with Ollama/));
      });

      await waitFor(() => {
        expect(mockUpsert).toHaveBeenCalledWith('MTS_PROVIDER', 'ollama', false);
        expect(mockUpsert).toHaveBeenCalledWith('MTS_MODEL', 'gpt-oss:20b', false);
