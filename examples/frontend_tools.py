...
MTS_HOST = "127.0.0.1"
MTS_PORT = "3001"
MTS_URL = f"http://{MTS_HOST}:{MTS_PORT}"
...
async def setup_agent() -> None:
    ...
    async with httpx.AsyncClient() as client:
        ...
        response = await client.post(
            f"{MTS_URL}/agent/update_provider",
            json={"provider": "databricks", "model": "mts"},
...
def submit_tool_result(tool_id: str, result: List[Dict[str, Any]]) -> None:
    """Submit the tool execution result back to MTS.
...
async def chat_loop() -> None:
    """Main chat loop that handles the conversation with MTS."""
