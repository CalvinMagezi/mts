...
I recently put this approach into practice with [mts](/), migrating a legacy LLM credit provisioning system split across two repositories (React/Vite frontend and Node/Express backend) into a unified Next.js framework.
...
I hopped on a livestream to tackle this huge refactor but seconds in I realized I realistically could not do this all in one hour. I focused on consolidating the fragmented codebase first. Two repositories (React/Vite frontend, Express backend) needed to become one monolithic Next.js application. But simply telling mts "Convert to Next.js" wouldn't work without proper context building.

## My Prompt Strategy

### Building a Shared Mental Model

Before I instructed mts to write any code, I prioritized helping it understand the codebase systematically with the following prompt:

> Can you get a lay of the land for the two projects found here and how they communicate?

mts employed the [analyze tool](/docs/mcp/developer-mcp#developer-extension-tools) to generate a high-level architectural flow. (The analyze tool is part of the [Developer extension](/docs/mcp/developer-mcp), an [MCP server](https://modelcontextprotocol.io/docs/learn/server-concepts) that's built into mts).

```
User Browser
    ↓
[mts-access-gateway] (React SPA)
    ↓ (HTTPS REST API)
[mts-hacknight-backend] (Express API)
    ↓ (HTTPS REST API)
...
Yes, I could have found the commands in the package.json, but asking mts to do it served a purpose: it grounded mts in the actual project setup and prevented it from hallucinating commands or ports.

:::tip Pro tip
I always have mts tell me what commands to run (like npm run dev) rather than executing them itself. Long-running or blocking commands can halt mts's process.
:::
...
:::note
In retrospect, the instruction regarding blocking commands should have been codified in [persistent context files](/docs/guides/using-mtshints) ([AGENTS.md](https://agents.md/) or [mtshints](/docs/guides/using-mtshints)) rather than inline prompts.
:::

But, I was so happy that mts generated a pixel perfect recreation of the app. 

### Task Decomposition

The agent's successful, perfect recreation of the UI was largely due to the [Todo extension](/docs/mcp/todo-mcp), an MCP server that's built into mts. I find that this extension helps prevent scope drift, where agents autonomously expand into adjacent functionality after completing an objective.
...
When I ran the app locally, I did encounter a Tailwind CSS v4/v3 syntax error, but mts used the Chrome Dev Tools extension and the Todo extension to quickly fix it.
...
To solve this problem, I instituted an automated commit policy by adding a persistent directive to my .mtshints file:

> Every time you make a change, make a commit using the GitHub CLI or the GitHub MCP Server.

### Pattern Replication

The final step was to add the backend logic for emailing API keys. Instead of asking mts to invent this from scratch, I had it learn from a known-working system: a separate app with similar provisioning logic.

I gave mts the following prompt:
...
> Use what you learned from the recipe project logic to make this happen in mts-credits... send the API key to their email using the SendGrid API.

This "copy-and-adapt" strategy was incredibly effective. mts successfully implemented the necessary API routes and clearly identified the environment variables I needed to supply. I manually added those variables. I didn't give them to mts for security purposes.

## The Lesson

I shared my messy, tedious conversation with mts (using Claude Sonnet 4.5) so that readers can confidently ditch one-shot prompts for complex tasks and work incrementally with agents. Just like coding, collaborating with an agent requires patience, but it doesn't have to feel stressful. 
...
*Ready to try AI-assisted migration with mts? Get started with our [quickstart guide](/docs/quickstart) and share your experience in our [Discord community](http://discord.gg/mts-oss).*


<head>
  <meta property="og:title" content="How to Successfully Migrate Your App with an AI Agent" />
  <meta property="og:type" content="article" />
  <meta property="og:url" content="https://block.github.io/mts/blog/2025/11/17/migrate-app-with-ai-agent" />
  ...
  <meta property="og:image" content="https://block.github.io/mts/assets/images/migrate-app-ai-agent-e8e3dcddf74909b6f84f85c8c776aaed.png" />
  ...
  <meta name="twitter:image" content="https://block.github.io/mts/assets/images/migrate-app-ai-agent-e8e3dcddf74909b6f84f85c8c776aaed.png"/>
</head>
