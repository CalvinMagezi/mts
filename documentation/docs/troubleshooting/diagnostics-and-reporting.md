...
description: Use built-in diagnostics, report bugs, and request new features with mts's integrated support tools.
---
...
mts provides several built-in features to help you get support, report issues, and request new functionality. This guide covers the diagnostics system, bug reporting, and feature request tools.
...
<Tabs groupId="interface">
  ...
  <TabItem value="cli" label="mts CLI">
    Use the session diagnostics command to generate a troubleshooting bundle. For complete details and all available options, see the [CLI Commands guide](/docs/guides/mts-cli-commands#session-diagnostics-options).

    ```sh
    # Generate diagnostics for a specific session
    mts session diagnostics --session-id <session_id>

    # Interactive selection (prompts you to choose a session)
    mts session diagnostics

    # Save to a custom location
    mts session diagnostics --session-id <session_id> --output /path/to/diagnostics.zip
    ```
...
├── logs/
│   ├── mts-2024-01-15.jsonl
│   ├── mts-2024-01-14.jsonl
│   └── ...
...
The feature request system helps you suggest improvements and new functionality for mts.
...
## Error Recovery with "Ask mts"

When certain types of error occur in mts Desktop (such as failures to activate extensions), you'll see an `Ask mts` button in the error notification. This feature lets you quickly troubleshoot the issue with mts's help:

1. When the error occurs, an `Ask mts` button appears in the error notification
2. Click the button to send the error details to mts in a chat prompt
3. mts provides diagnostic suggestions and potential solutions
