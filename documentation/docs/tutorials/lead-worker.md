...
The lead/worker model is a smart hand-off system. The "lead" model (think: GPT-4 or Claude Opus) kicks things off, handling the early planning and big picture reasoning. Once the direction is set, mts hands the task over to the "worker" model (like GPT-4o-mini or Claude Sonnet) to carry out the steps.

If things go sideways (e.g. the worker model gets confused or keeps making mistakes), mts notices and automatically pulls the lead model back in to recover. Once things are back on track, the worker takes over again.

## Turn-Based System

A **turn** is one full interaction - your prompt and the model's response. mts switches models based on turns:
...
You might configure mts like this:

```bash
export MTS_LEAD_MODEL="gpt-4o"          # strong reasoning
export MTS_MODEL="gpt-4o-mini"          # fast execution
export MTS_PROVIDER="openai"
```

mts will start with `gpt-4o` for the first three turns, then hand off to `gpt-4o-mini`. If the worker gets tripped up twice in a row, mts temporarily switches back to the lead model for two fallback turns before trying the worker again.

## Configuration

:::tip
Ensure you have [added the LLMs to mts](/docs/getting-started/providers)
:::

<Tabs groupId="interface">
  <TabItem value="ui" label="mts Desktop" default>
   1. Click the model name at the bottom of the mts Desktop window
  ...
  <TabItem value="cli" label="mts CLI">
    The only required configuration is setting the `MTS_LEAD_MODEL` [environment variable](/docs/guides/environment-variables#leadworker-model-configuration):

    ```bash
    export MTS_LEAD_MODEL="gpt-4o"
    ```
    That's it. mts treats your regular `MTS_MODEL` as the worker model by default.

    For more control, you can also set these optional environment variables:

    ```bash
    export MTS_LEAD_PROVIDER="anthropic"         # If different from the main provider
    export MTS_LEAD_TURNS=5                      # Use lead model for first 5 turns
    export MTS_LEAD_FAILURE_THRESHOLD=3          # Switch back to lead after 3 failures
    export MTS_LEAD_FALLBACK_TURNS=2             # Use lead model for 2 turns before retrying worker
    ```
...
mts is smart about detecting actual task failures, not just API errors. The fallback kicks in when the worker:
...
Technical hiccups like timeouts, authentication issues, or service downtime don't trigger fallback mode. mts retries those quietly.
...
- Bump up `MTS_LEAD_TURNS` to 5–7 for heavier planning upfront
- Lower `MTS_LEAD_FAILURE_THRESHOLD` to 1 if you want mts to correct issues quickly
...
```bash
export RUST_LOG=mts::providers::lead_worker=info
```

## Planning Mode Compatibility
The lead/worker model is an automatic alternative to the [mts CLI's `/plan` command](/docs/guides/creating-plans). You can assign separate models to use as the lead/worker and planning models. For example:

```bash
export MTS_PROVIDER="openai"
export MTS_MODEL="gpt-4o-mini"        # the main conversational model

export MTS_LEAD_MODEL="o1-preview"    # the lead model used automatically
export MTS_PLANNER_MODEL="gpt-4o"     # the model used when you explicitly call /plan
```
