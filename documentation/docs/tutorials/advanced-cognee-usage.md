---
title: Advanced Cognee Usage with mts
description: Advanced patterns for using Cognee knowledge graph with mts for enhanced memory and automation
---

import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

# Advanced Cognee Usage with mts

This tutorial covers advanced usage patterns for the Cognee extension with mts, including automated memory management, knowledge graph optimization, and various integration strategies.

## Overview

While the basic [Cognee MCP setup](../mcp/cognee-mcp.md) gets you started, this tutorial explores how to make mts autonomously use the knowledge graph and optimize your workflow.
...
Create `~/.config/mts/cognee-instructions.md`:
...
Start mts with instructions:
```bash
mts run -i ~/.config/mts/cognee-instructions.md -s
```
...
For faster startup with higher token usage, add to your `.mtshints` file:
...
3. Lower token usage than mtshints
...
```bash
# Start mts with Cognee
mts session

# In mts, analyze your codebase
> mts, please codify this repository and then help me understand the architecture
```
...
> mts, please cognify the contents of these research papers: paper1.pdf, paper2.pdf, paper3.pdf
...
```bash
# Check status
> mts, what's the status of the cognify pipeline?

# Selective pruning (if needed)
> mts, can you help me identify outdated information in the knowledge graph?
```
...
4. **High token usage**: Use instruction files instead of mtshints
...
# In mts session:
> mts, run cognify_status and codify_status
```
...
1. **Use nodesets** for organizing different types of information:
   ```bash
   # Developer rules
   > mts, add these coding standards to the 'developer_rules' nodeset
   
   # Project-specific info  
   > mts, cognify this project documentation with nodeset 'project_alpha'
   ```
...
```bash
# Setup
> mts, codify this repository and remember that I prefer: functional programming patterns, comprehensive tests, and clear documentation
...
```bash
# Before meeting
> mts, cognify the agenda and participant backgrounds from these documents
...
```bash
# Literature review
> mts, cognify these 10 research papers and create a knowledge graph of the relationships between their methodologies

# Synthesis
> What are the emerging patterns in the research and what gaps exist?
```

This advanced usage guide should help you maximize the potential of Cognee with mts for sophisticated knowledge management and automation workflows.
