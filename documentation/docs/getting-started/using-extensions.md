...
Extensions are add-ons that provide a way to extend the functionality of mts by connecting with applications and tools you already use in your workflow. These extensions can be used to add new features, access data and resources, or integrate with other systems.

Extensions are based on the [Model Context Protocol (MCP)](https://github.com/modelcontextprotocol), so you can connect
mts to a wide ecosystem of capabilities.

mts automatically checks external extensions for known malware before activation. If a malicious package is detected, the [extension will be blocked](/docs/troubleshooting/known-issues#malicious-package-detected) with a clear error message.

:::tip Tutorials
Check out the [step-by-step tutorials](/docs/category/mcp-servers) for adding and using various mts extensions
:::

## Built-in Extensions
mts includes several built-in extensions you can start using out of the box:
...
- [Memory](/docs/mcp/memory-mcp): Teaches mts to remember your preferences as you use it.
- [Tutorial](/docs/mcp/tutorial-mcp): Provides interactive tutorials for learning about mts.
- [Auto Visualiser](/docs/mcp/autovisualiser-mcp): Automatically generates graphical data visualizations in conversations.

:::warning Access Control
mts operates autonomously by default. Combined with the Developer extension's tools, this means mts can execute commands and modify files without your approval. If you want more control over this behavior, you can configure the [mts permission mode](/docs/guides/mts-permissions), [tool permissions](/docs/guides/managing-tools/tool-permissions), and [.mtsignore files](/docs/guides/using-mtsignore). See [Configuring Access Controls](/docs/mcp/developer-mcp#configuring-access-controls) for a brief overview.
:::
...
:::info
mts's built-in extensions are MCP servers in their own right. If you'd like
to use the MCP servers included with mts with any other agent, you are free to do so.
:::

## Discovering Extensions

mts provides a [central directory][extensions-directory] of extensions that you can install and use. 

You can also add any other [MCP Server](#mcp-servers) as a mts extension, even if it's not listed in our directory.
...
You can install any MCP server as a mts extension. 
...
<Tabs groupId="interface">
  ...
  4. On the `Add custom extension` modal, enter the necessary details
     ...
     - The `Timeout` field lets you set how long mts should wait for a tool call from this extension to complete
  5. Click `Add` button
  ...
  3. Choose the type of extension you'd like to add:
      - `Built-In Extension`: Use an extension that comes pre-installed with mts.
...
Extensions can be installed using mts's deep link protocol. The URL format varies based on the extension type:

<Tabs groupId="interface">
  <TabItem value="stdio" label="StandardIO" default>
```
mts://extension?cmd=<command>&arg=<argument>&id=<id>&name=<name>&description=<description>
```

Required parameters:
- `cmd`: The base command to run, one of `jbang`, `npx`, `uvx`, `mtsd`, or `docker`
...
```
mts://extension?cmd=npx&arg=-y&arg=%40modelcontextprotocol/server-github&timeout=<timeout>&id=<id>&name=<name>&description=<description>
```
...
```
mts://extension?url=<remote-sse-url>&id=<id>&name=<name>&description=<description>
```
...
```
mts://extension?url=http%3A%2F%2Flocalhost%3A8080%2Fsse&timeout=<timeout>&id=<id>&name=<name>&description=<description>>
```

  </TabItem>
  <TabItem value="streamable_http" label="Streaming HTTP">
```
mts://extension?url=<remote-streamable-http-url>&type=streamable_http&id=<id>&name=<n>&description=<description>
```
...
```
mts://extension?url=https%3A%2F%2Fexample.com%2Fstreamable&type=streamable_http&timeout=<timeout>&id=<id>&name=<n>&description=<description>
```
...
### Config Entry
For advanced users, you can also directly edit the config file (`~/.config/mts/config.yaml`) to add, remove, or update an extension:
...
<Tabs groupId="interface">
  ...
  <TabItem value="cli" label="mts CLI">
    1. Run the following command to open up mts's configurations:
    ```sh
    mts configure
    ```
...
The Smart Extension Recommendation system in mts automatically identifies and suggests relevant extensions based on your tasks and needs. This section explains how to use this feature effectively and understand its capabilities and limitations.

When you request a task, mts checks its enabled extensions and their tools to determine if it can fulfill the request. If not, it suggests or enables additional extensions as needed. You can also request specific extensions by name.
...
mts automatically detects when an extension is needed based on your task requirements. Here's an example of how mts identifies and enables a needed extension during a conversation:
...
■  mts would like to enable the following extension, do you approve?
// highlight-start
...
mts responds to explicit requests for extensions, allowing users to manually enable specific tools they need. Here's an example of how mts handles a direct request to enable an extension:
...
mts relies on extension properties to determine how to handle an extension. You can edit these properties if you want to change the extension's display settings and behavior, such as the name, timeout, or environment variables.
...
<Tabs groupId="interface">
  ...
  1. Navigate to the mts [configuration file](/docs/guides/config-files). For example, navigate to `~/.config/mts/config.yaml` on macOS.
...
You can start a tailored mts session with specific extensions directly from the CLI. 
...
```bash
mts session --with-builtin "{extension_id}"
```

For example, to enable the Developer and Computer Controller extensions and start a session, you'd run:

```bash
mts session --with-builtin "developer,computercontroller"
```

Or alternatively:

```bash
mts session --with-builtin developer --with-builtin computercontroller
```
...
```bash
mts session --with-extension "{extension command}" --with-extension "{another extension command}"
```
...
```bash
mts session --with-extension "uvx mcp-server-fetch"
```
...
```bash
mts session --with-extension "VAR=value command arg1 arg2"
```

For example, to start a session with the [GitHub extension](https://github.com/github/github-mcp-server), you'd run:

```bash
mts session --with-extension "GITHUB_PERSONAL_ACCESS_TOKEN=<YOUR_TOKEN> npx -y @modelcontextprotocol/server-github"
```
...
```bash
mts session --with-remote-extension "{extension URL}" --with-remote-extension "{another extension URL}"
```

For example, to start a session with a remote extension over SSE running on localhost on port 8080, you'd run:

```bash
mts session --with-remote-extension "http://localhost:8080/sse"
```
...
```bash
mts session --with-streamable-http-extension "{extension URL}" --with-streamable-http-extension "{another extension URL}"
```

For example, to start a session with a Streaming HTTP extension, you'd run:

```bash
mts session --with-streamable-http-extension "https://example.com/streamable"
```

## Developing Extensions

mts extensions are implemented with MCP, a standard protocol that allows AI models and agents to securely connect with local or remote resources. Learn how to build your own [extension as an MCP server](https://modelcontextprotocol.io/quickstart/server).
